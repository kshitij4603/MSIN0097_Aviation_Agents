...
Programmatically builds MSIN0097_Comparative_Analysis.ipynb using nbformat.
"""
...
cell_4 = new_markdown_cell("""\
### EDA Visual Evidence

> **Note on plot availability:** Only Claude saved rendered PNG artefacts to its output directory. Codex and Antigravity produced EDA scripts that generate plots at runtime but did not persist the images to disk. The Claude plots below are therefore the sole embeddable visual evidence at this stage; Codex and Antigravity plots must be regenerated by executing their respective scripts.
...
#### Codex — EDA Script (plots rendered at runtime)
`agent_outputs/codex/eda_delay_drivers.py` — run locally to reproduce.

---

#### Antigravity — EDA Script (plots rendered at runtime)
`agent_outputs/antigravity/eda.py` — run locally to reproduce.\
""")
...
nb = new_notebook()
nb.cells = [cell_1, cell_2, cell_3, cell_4]

# Set kernel metadata so Jupyter recognises it as Python 3
nb.metadata = {
    "kernelspec": {
...
output_path = "MSIN0097_Comparative_Analysis.ipynb"
...
for i, cell in enumerate(nb.cells, 1):
    preview = cell.source.splitlines()[0][:80]
    print(f"     Cell {i} ({cell.cell_type}): {preview!r}")
